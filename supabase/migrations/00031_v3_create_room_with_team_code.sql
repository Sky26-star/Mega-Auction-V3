-- Atomic room creation and team creation for Host, updated with p_room_code
...
BEGIN
  ...
  -- 1. Create Room (now includes code)
...
REVOKE ALL ON FUNCTION public.create_room_with_team(TEXT, TEXT, JSONB, UUID, TEXT, TEXT, TEXT, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_room_with_team(TEXT, TEXT, JSONB, UUID, TEXT, TEXT, TEXT, INT) TO authenticated, service_role;

-- Drop the old function signature so it doesn't conflict
DROP FUNCTION IF EXISTS public.create_room_with_team(TEXT, JSONB, UUID, TEXT, TEXT, TEXT, INT);
