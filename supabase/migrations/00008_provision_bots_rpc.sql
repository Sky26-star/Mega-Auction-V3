-- Migration: 00008_provision_bots_rpc.sql
-- Description: Trusted SECURITY DEFINER RPC for Atomic Provisioning of Bot Opponents
-- Target Database: PostgreSQL 15+ (Supabase)

CREATE OR REPLACE FUNCTION public.provision_room_bots(
  p_room_id UUID,
  p_bots JSONB DEFAULT '[]'::jsonb
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.provision_room_bots(UUID, JSONB) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.provision_room_bots(UUID, JSONB) TO authenticated, service_role;
