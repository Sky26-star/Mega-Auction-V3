-- Migration: 00019_fix_player_stats_and_timer.sql
-- Description: Authoritative Player Stats Exporter and Safe Fresh Timer Initialization
-- Target Engine: PostgreSQL 15+ (Supabase Cloud)

BEGIN;

-- ============================================================
-- 1. AUTHORITATIVE STATE EXPORTER WITH PLAYER STATS & ISO TIMESTAMPS
-- ============================================================
...
BEGIN
  ...
  -- Compute GET READY window & ISO timestamps for active lot
  IF v_active_lot.id IS NOT NULL THEN
    ...
    IF v_get_ready_expires_at IS NOT NULL THEN
      v_get_ready_expires_iso := to_char(v_get_ready_expires_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
    END IF;

    -- Fetch current player with full statistics
...
-- ============================================================
-- 2. SAFE & GUARANTEED INITIALIZATION: public.initialize_and_start_auction
-- ============================================================
...
BEGIN
  ...
  -- If auction is ALREADY IN_PROGRESS and lot 0 has active timer in future, return state
  IF v_auction.id IS NOT NULL AND v_auction.status = 'IN_PROGRESS' THEN
    SELECT id, timer_expires_at INTO v_first_lot_id, v_timer_expires
    FROM public.auction_lots
    WHERE auction_id = v_auction.id AND lot_index = v_auction.current_lot_index;

    IF v_timer_expires IS NOT NULL AND v_timer_expires > clock_timestamp() THEN
  ...
  -- Set fresh 3-second GET READY + timer_sec countdown buffer on Lot 0
  ...
  -- Update auction row to IN_PROGRESS & reset current_lot_index to 0 for Lot 0 start
