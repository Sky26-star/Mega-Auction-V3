-- Migration: 00027_v3_auction_foundation.sql
-- Description: Phase 2 Database Foundation for V3 Auction Engine.

-- 1. Modify auction_lots status to include GET_READY
ALTER TABLE public.auction_lots DROP CONSTRAINT IF EXISTS auction_lots_status_check;
ALTER TABLE public.auction_lots ADD CONSTRAINT auction_lots_status_check CHECK (status IN ('PENDING','GET_READY','ACTIVE','BIDDING','SOLD','UNSOLD','SKIPPED'));

-- 2. Add get_ready_expires_at field
ALTER TABLE public.auction_lots ADD COLUMN IF NOT EXISTS get_ready_expires_at TIMESTAMPTZ;
