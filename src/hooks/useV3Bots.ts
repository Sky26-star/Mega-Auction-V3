...
import { V3AuctionState, V3Team, V3Player } from '@/lib/v3-auction-types';

// ============================================================
// TYPES
// ============================================================

type BotPersonality = 'CONSERVATIVE' | 'BALANCED' | 'AGGRESSIVE' | 'OPPORTUNISTIC' | 'STRATEGIC';
type BotDecision = 'SKIP' | 'ENTER' | 'HESITATE' | 'DROP' | 'ONE_LAST_BID';
type RoleNeed = 'CRITICAL' | 'HIGH' | 'NORMAL' | 'LOW' | 'REDUNDANT';

interface FuturePlayer {
  role: string;
  category: string;
  base_price: number;
  is_overseas: boolean;
...
const DEFAULT_CONTEXT: AuctionContext = {
  maxSquadSize: 25,
  maxOverseas: 8,
  totalLots: 50,
  futurePlayers: [],
...
function getNextBidAmount(currentBid: number, basePrice: number): number {
...
function getBotPersonality(teamId: string): BotPersonality {
...
function calculateFranchiseOwnerValuation(
  bot: V3Team,
  player: V3Player,
  difficulty: string,
  allTeams: V3Team[],
...
// ============================================================
// useV3Bots HOOK
// Timer architecture, stable refs, and lifecycle protection
// are PRESERVED exactly from the previous fix.
// ============================================================
export function useV3Bots(state: V3AuctionState, isHost: boolean, botDifficulty: string, onBidSuccess?: () => void) {
  // ⚠️ DISABLED: Bot orchestration has been migrated to the server.
  // The server ticker now handles all authoritative bot bids and auction progression.
  // See `src/lib/auction/server-bot-engine.ts` for the business logic.
  return;
}
